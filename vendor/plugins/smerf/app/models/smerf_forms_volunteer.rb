...
class SmerfFormsVolunteer < ActiveRecord::Base
  ...
  #
  def SmerfFormsVolunteer.find_user_smerf_form(volunteer_id, smerf_form_id)
    # Find the form record for the current user
    smerf_forms_volunteer = nil
    if (volunteer_id > 0)
      smerf_forms_volunteer = SmerfFormsVolunteer.find(:first, 
        :conditions => ['smerf_form_id = ? AND volunteer_id = ?', 
        smerf_form_id, volunteer_id])
    else
      raise(RuntimeError, 
    ...
    return smerf_forms_volunteer
  end

  # Create a record for the user for the current form
  #
  def SmerfFormsVolunteer.create_records(smerf_form_id, volunteer_id, responses)
    transaction do
      # Create the record for the user
      smerf_forms_volunteer = SmerfFormsVolunteer.create(
      ...
      responses.each do |question_response|
        ...
        if (question_response[1].kind_of?(Hash) or 
          ...
          question_response[1].each do |multichoice_response|
            ...
            smerf_forms_volunteer.smerf_responses << SmerfResponse.new(
  ...
  #
  def SmerfFormsVolunteer.update_records(smerf_form_id, volunteer_id, responses)
    transaction do
      # Retrieve the response record for the current user
      smerf_forms_volunteer = SmerfFormsVolunteer.find_user_smerf_form(
        volunteer_id, smerf_form_id)
      if (smerf_forms_volunteer)
        # Update the record with the new responses 
        smerf_forms_volunteer.update_attribute(:responses, responses)
        ...
        current_responses = SmerfResponse.find(:all, 
          :conditions => ["smerf_forms_volunteer_id = ?", smerf_forms_volunteer.id])  
              ...
              if (!multichoice_response[1].blank?())
                ...
                if (found = self.response_exist?(current_responses, 
                  ...
                  new_responses << (SmerfResponse.new(
                    :smerf_forms_volunteer_id => smerf_forms_volunteer.id, 
                    :question_code => question_response[0],
        ...
        smerf_forms_volunteer.smerf_responses.replace(new_responses)
      end
    end
  end
  ...
  private
    ...
    #
    def SmerfFormsVolunteer.response_exist?(current_responses, question_code, response)
      response_object = nil
      begin
